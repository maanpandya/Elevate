...
ttruss_min, ttruss_max = 0.0004, 0.003  # Thickness range for truss (m)
...
weight_deflection = 0.2
weight_mass = 0.5
weight_stress = 0.2
weight_buckling = 0.1

# Function to calculate buckling stress
def calc_sigma_x(t, D, E, v):
    f = 1/16 * np.sqrt((0.5 * D) / t)
    y = 1 - 0.901 * (1 - np.exp(-f))
    sigma_x = (y * E / (np.sqrt(3 * (1 - v**2)))) * (t / (0.5 * D))
    return sigma_x
                ...
                for dtruss in np.arange(dtruss_min, dtruss_max, 0.01):
                    for t in np.arange(t_min, t_max, 0.001):
                        for d in np.arange(d_min, d_max, 0.01):
                            ...
                            sigma_x = calc_sigma_x(ttruss, dtruss, E, v)
