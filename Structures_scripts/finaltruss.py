...
# Function to calculate buckling stress
...
# Truss configuration loop function
def truss_config_loop(T, L, sigmaY, rho, E, v, t_min, t_max, d_min, d_max, ttruss_min, ttruss_max, dtruss_min, dtruss_max):
    min_score = float('inf')
    best_params = None
...
# No truss configuration loop function
def no_truss_config_loop(T, L, sigmaY, rho, E, t_min, t_max, d_min, d_max):
    min_score = float('inf')
    best_params = None
...
def optimize_structure(T=833, L=1.8, sigmaY=280, rho=2770, htruss=0.5, E=71*10**9, v=0.33, 
                       ...
                       ttruss_min=0.001, ttruss_max=0.009, dtruss_min=0.07, dtruss_max=0.071, 
                       weight_deflection=0.1, weight_mass=0.5, weight_stress=0.1, weight_buckling=0.3, 
                       truss=2):

    if truss == 2:
        best_params, min_score = truss_config_loop(T, L, sigmaY, rho, E, v, t_min, t_max, d_min, d_max, 
                                                   ttruss_min, ttruss_max, dtruss_min, dtruss_max)
    else:
        best_params, min_score = no_truss_config_loop(T, L, sigmaY, rho, E, t_min, t_max, d_min, d_max)
...
result = optimize_structure(truss=configuration)
